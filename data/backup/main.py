...
if __name__ == '__main__':
    ...
    if parameter["mode"] == "train":
        ...
        #train_extern_data = extern_data[:]
        train_extern_data, dev_extern_data, test_extern_data = extern_data[:-dev_size], extern_data[-dev_size:-test_size], extern_data[-test_size:]
        # dataset.make_input_data(train_extern_data)
        dev_dataset.make_input_data(dev_extern_data)  # For Dev set
        test_dataset.make_input_data(test_extern_data) # for Test set
        ...
        # Check for test set
        parameter["mode"] = "test"
        te_avg_cost, te_avg_correct, te_precision_count, te_recall_count = iteration_model(model, test_dataset,
                                                                                           parameter, train=False)
        print('[Epoch: {:>4}] cost = {:>.6} Accuracy = {:>.6}'.format(epoch + 1, te_avg_cost, te_avg_correct))
        te_f1Measure, te_precision, te_recall = calculation_measure(te_precision_count, te_recall_count)
        print('[Test] F1Measure : {:.6f} Precision : {:.6f} Recall : {:.6f}'.format(te_f1Measure, te_precision, te_recall))
